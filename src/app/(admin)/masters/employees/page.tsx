...
import {
  deleteOrphanedAuthUserAction,
  diagnoseEmployeeStateAction,
...
import { mapEmployeeFromDb } from "../../../../features/context/DataContext";
...
export default function EmployeeListPage() {
  ...
  const isSetupMode =
    ...
    document.cookie.includes("is_initial_setup=true");
...
function EmployeeListContent() {
  ...
  const handleExportCSVClick = async () => {
    ...
    try {
      const allMatchingRaw = await fetchEmployeesAllAction(searchTerm);
      const allMatching = allMatchingRaw.map(mapEmployeeFromDb);

      handleExport(
        allMatching,
        headers,
        `employee_list_${new Date().toISOString().split("T")[0]}.csv`,
        (item) => {
          const [lastName, ...firstNameParts] = (item.name || "").split(
            /[\s　]+/,
          );
          const [lastNameKana, ...firstNameKanaParts] = (
            item.nameKana || ""
          ).split(/[\s　]+/);
  ...
  // Highlight effect
  const rowClassName = (item: Employee) =>
      ...
      <Table<Employee>
        ...
        columns={[
          ...
          { header: "氏名カナ", accessor: "nameKana" },
          {
            ...
            accessor: (item) => (
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${item.role === "admin" ? "bg-purple-100 text-purple-800" : "bg-gray-100 text-gray-800"}`}
              >
                {item.role === "admin" ? "管理者" : "ユーザー"}
              </span>
            ),
