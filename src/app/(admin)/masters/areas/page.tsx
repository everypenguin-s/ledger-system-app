...
import { mapAreaFromDb } from "../../../../features/context/DataContext";
...
function AreaListContent() {
  ...
  const handleExportCSVClick = async () => {
    ...
    await logger.log({
      ...
      targetId: "area_list",
      result: "success",
      message: `エリアマスタのエクスポート開始`,
    });

    try {
      const allMatchingRaw = await fetchAreasAllAction(searchTerm);
      const allMatching = allMatchingRaw.map(mapAreaFromDb);

      handleExport(
        allMatching,
        headers,
        `area_list_${new Date().toISOString().split("T")[0]}.csv`,
        (item) => [item.areaCode, item.areaName],
      );
    } catch (error) {
