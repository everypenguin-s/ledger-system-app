...
import {
  ...
  fetchAddressesAllAction,
} from "../../../../app/actions/master_fetch";
import { mapAddressFromDb } from "../../../../features/context/DataContext";
...
function AddressListContent() {
  ...
  const [editingItem, setEditingItem] = useState<Address | undefined>(
    undefined,
  );
  ...
  const handleExportCSVClick = async () => {
    ...
    await logger.log({
      ...
      result: "success",
      message: `事業所マスタのエクスポート開始`,
    });

    try {
      const allMatchingRaw = await fetchAddressesAllAction(searchTerm);
      const allMatching = allMatchingRaw.map(mapAddressFromDb);

      handleExport(
        allMatching,
        headers,
        `address_list_${new Date().toISOString().split("T")[0]}.csv`,
        (item) => {
          return [
            item.addressCode || "",
            item.officeName || "",
  ...
  // Highlight effect
  const rowClassName = (item: Address) =>
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
        ...
        columns={[
          ...
          },
          {
            header: (
              ...
              >
                事業所コード{getSortIcon("addressCode")}
              </div>
            ),
            accessor: (item) => (
              <button
          ...
          { header: "事業所名", accessor: "officeName" },
          {
            header: (
              <div
                className="flex items-center cursor-pointer"
                onClick={() => toggleSort("tel")}
              >
                ＴＥＬ{getSortIcon("tel")}
              </div>
            ),
            accessor: (item) => formatPhoneNumber(item.tel),
          },
          {
            header: (
              <div
                className="flex items-center cursor-pointer"
                onClick={() => toggleSort("fax")}
              >
                ＦＡＸ{getSortIcon("fax")}
              </div>
            ),
            accessor: (item) => formatPhoneNumber(item.fax),
          },
          {
            header: (
              <div
            ...
            accessor: (item) => formatZipCode(item.zipCode),
          },
      ...
      <Modal
        ...
        title={editingItem ? "事業所 編集" : "事業所 新規登録"}
      >
        <AddressForm
