...
import { mapRouterFromDb } from "../../../../features/context/DataContext";
...
function RouterListContent() {
  ...
  const handleExportCSVClick = async () => {
    ...
    try {
      const allMatchingRaw = await fetchRoutersAllAction(searchTerm);
      const allMatching = allMatchingRaw.map(mapRouterFromDb);
      handleExport(
        allMatching,
        headers,
        `wifi_router_list_${new Date().toISOString().split("T")[0]}.csv`,
        (item) => [
          item.terminalCode,
  ...
  const getSortIcon = (key: keyof Router | "userName") => {
    // Cast
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
        ...
        columns={[
          ...
          },
          {
            header: (
              <div
                ...
                onClick={() => toggleSort("terminalCode")}
              >
                端末CD{getSortIcon("terminalCode")}
              </div>
            ),
            accessor: (item) => (
              <button
          ...
          },
          {
            header: (
              <div
                className="flex items-center cursor-pointer"
                onClick={() => toggleSort("simNumber")}
              >
                SIM電番{getSortIcon("simNumber")}
              </div>
            ),
            accessor: (item) => formatPhoneNumber(item.simNumber || ""),
          },
          {
            header: (
              <div
      ...
      <Modal
        ...
        title={editingItem ? "ルーター 編集" : "ルーター 新規登録"}
      >
        <RouterForm
